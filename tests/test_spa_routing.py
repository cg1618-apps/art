...
def test_a_registered_route_still_reaches_its_own_handler(tmp_path):
    ...
    # the SPA's HTML.
    response = TestClient(_app_with_dist(tmp_path)).get("/health")
    assert not response.headers["content-type"].startswith("text/html")


def test_an_unknown_health_path_is_404_not_the_spa(tmp_path):
    # This app's health path is /health, not /api/health, so the `api` guard
    # alone would let a mistyped probe - /health/ready, /healthz under the
    # prefix - be answered by the SPA with a 200. A probe that cannot fail is
    # worse than no probe, and the deploy pipeline waits on exactly this path.
    response = TestClient(_app_with_dist(tmp_path)).get("/health/ready")
    assert response.status_code == 404
...
def test_a_non_api_path_serves_the_spa_index(tmp_path):
    ...
    response = TestClient(_app_with_dist(tmp_path)).get("/records/42")
...
def test_a_bundle_with_no_assets_directory_still_starts(tmp_path):
    ...
    (tmp_path / "index.html").write_text("<html><body>spa</body></html>")
    response = TestClient(create_app(dist=tmp_path)).get("/records/42")
    assert response.status_code == 200
    assert "spa" in response.text
