...
This app has no uploads yet - references and finished works will need them,
and the platform has not generalised image storage - so there is nothing to
mount. The test below still asserts the shape any future mount must have:
a DIRECTORY, never a single file.
"""
...
def test_the_app_joins_the_shared_network_as_art_app(compose):
    # This alias is the contract with the generated ingress in
    # cg1618-apps/platform, which routes art.cg1618.com to
    # http://art-app:8003. Change it here alone and the hostname 502s while
    # both files still read as correct on their own.
    app_networks = compose["services"]["app"]["networks"]
    assert app_networks["cg1618"]["aliases"] == ["art-app"]
    assert compose["services"]["app"]["environment"]["PORT"] == 8003
...
def test_the_app_healthcheck_does_not_probe_the_catch_all_route(compose):
    ...
    #
    # /health retires that reasoning by opening a real session, reading
    ...
    #
    # The path is /health and not /api/health because apps.yml declares this
    # app's health_path as /health, and the deploy pipeline reads the path it
    # waits on from there. Compose and the registry must agree.
    #
    ...
    assert f":{port}/health" in probe, probe
...
def test_app_carries_an_image_name_alongside_build(compose):
    ...
    assert app["image"] == "art-app:local"


def test_the_app_has_no_bind_mounts(compose):
    # This app has no uploads yet, so a bind mount here would create an empty
    # directory nobody writes to. When references and works do need one it
    # must mount a DIRECTORY: docker silently creates a directory in place of
    # a missing single-file source, and the container then reads it as an
    # empty file rather than failing.
    for service in compose["services"].values():
        for mount in service.get("volumes", []):
            source = mount["source"] if isinstance(mount, dict) else mount.split(":")[0]
            assert not Path(source).suffix, f"single-file bind mount: {mount}"
