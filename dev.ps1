...
#
# Ported from media/dev.ps1, changed only where art's arrangement differs:
# this app has no docker-compose.yml of its own - the container is
# anime_site_postgres_db, owned by media's compose project, with an `art`
...
# --- Guard: a leftover backend on :8003 makes the new uvicorn die with WinError 10048,
# --- which surfaces only as vite "ECONNREFUSED 127.0.0.1:8003" proxy errors. Ports are
...
foreach ($i in 1..120) {
    try {
        Invoke-WebRequest "http://127.0.0.1:$backendPort/health" -UseBasicParsing -TimeoutSec 2 | Out-Null
        $apiReady = $true; break
    } catch {
