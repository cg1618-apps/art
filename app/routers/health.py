...
It is `/health`, not `/api/health`: `apps.yml` declares this app's health_path
as `/health`, and the deploy pipeline reads the probe's path from there. The
two must agree, or a deploy waits on a path that 404s and rolls itself back.
