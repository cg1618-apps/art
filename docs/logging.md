...
art's half of a contract that belongs to the platform. The contract itself is
`cg1618-apps/platform`'s `docs/logging.md` — the field names, the formats, the
request-id rules — and this page says how art implements it and what is
specific to art. **Where the two disagree, the contract wins**, because the
...
```json
{"timestamp":"2026-09-20T13:48:53.214+00:00","level":"INFO","logger":"app.routers.health","message":"loaded 3 items","app":"art","request_id":"9f2c..."}
```
...
## `app` is `"art"`, and the container is `art-app-1`
...
empties it outside a container; renaming the container breaks the `art-app`
network alias the generated ingress routes to, and `art.cg1618.com` answers 502
...
Without that, art emits a **half-structured stream**: uvicorn installs its own
...
## The inbound `X-Request-ID` is validated, even though art is gated

art is `exposure: cloudflare-access`, so unlike `media` and `food` this header
